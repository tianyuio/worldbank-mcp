...
let COUNTRIES_CACHE = [];
let INDICATORS_CACHE = [];
// World Bank API request function with enhanced error handling
async function makeWorldBankRequest(endpoint, params = {}) {
    try {
        const defaultParams = {
            format: 'json',
            per_page: 50,
...
async function getCompleteList(endpoint, params = {}, maxPages = 20, // Safety limit to avoid excessive requests
perPage = 100 // Reasonable page size
    ...
    try {
        while (currentPage <= maxPages) {
            const response = await makeWorldBankRequest(endpoint, {
                ...params,
                page: currentPage,
                per_page: perPage
            });
            ...
            const pageData = response[1];
            allData = allData.concat(pageData);
            // Extract pagination info from first array item
            const paginationInfo = response[0];
...
async function getCountries() {
    ...
    const allCountries = await getCompleteList('countries', {}, 5, 100);
    if (allCountries.length > 0) {
        COUNTRIES_CACHE = allCountries.filter((country) => country.region.value !== 'Aggregates' && country.id.length === 3);
        return COUNTRIES_CACHE;
    }
    return [];
}
...
async function getIndicators() {
    ...
    const allIndicators = await getCompleteList('indicators', {}, 10, 50);
...
// Get country data
async function getCountryData(countryCode, indicatorId, years = 10) {
    const response = await makeWorldBankRequest(`country/${countryCode}/indicator/${indicatorId}`, { per_page: years });
    if (response && Array.isArray(response[1])) {
        return response[1].filter((point) => point.value !== null);
    }
    return [];
}
...
function formatIndicatorData(data, indicatorName) {
    if (data.length === 0) {
...
async function handleIndicatorData(countryCode, indicatorKey, years) {
    const indicatorId = COMMON_INDICATORS[indicatorKey];
    if (!indicatorId) {
        return {
            content: [{
                    type: 'text',
    ...
    const country = countries.find(c => c.iso2Code === countryCode.toUpperCase() || c.id === countryCode.toUpperCase());
    if (!country) {
        return {
            content: [{
                    type: 'text',
...
server.tool('get-countries', 'Get all countries supported by World Bank', {
    ...
    const result = filteredCountries.map(country => `${country.name} (${country.iso2Code}) - ${country.region.value} - ${country.incomeLevel.value}`).join('\\n');
    return {
        content: [{
                type: 'text',
...
server.tool('get-country-info', 'Get detailed information for a specific country', {
...
server.tool('search-indicators', 'Search for available indicators', {
    keyword: z.string().describe('Search keyword, e.g.: "education", "health", "environment"')
}, async ({ keyword }) => {
    const indicators = await getIndicators();
    const filteredIndicators = indicators.filter(indicator => indicator.name.toLowerCase().includes(keyword.toLowerCase()) ||
        indicator.id.toLowerCase().includes(keyword.toLowerCase())).slice(0, 20); // Limit results
    ...
    const result = filteredIndicators.map(indicator => `${indicator.id}: ${indicator.name} (${indicator.unit || 'N/A'})`).join('\\n');
    return {
        content: [{
                type: 'text',
                text: result
...
toolConfigs.forEach(config => {
    server.tool(config.name, config.description, {
